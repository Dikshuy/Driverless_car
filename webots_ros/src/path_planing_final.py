...
vinit, ainit, VMAX, num_foc, horizon, follow_dis, critical_dis, v_obs = 0, 0, 5.0, 10, 6, 10, 10, 0
dyn_x, dyn_y =  x[np.size(x) - 1], y[np.size(y) - 1]
pre_state, state, a_i, t0, x_set_init, y_set_init, lane_status = 0, 0, 1, 0, 0, 0, 0
j, DISTANCE, Curv_const = 0, 25, 0.05
epsilon = 1e-05
...
curvature = np.zeros([num_foc+1,1])
################## Controller gains###################
c1, c2, c3 = 2, 0.01, 1.1
#*********************************************************************************************************************************************
...
if __name__ == "__main__":
    ...
    yaw, xact, yact, vact, wact = 0.0, 3.93, -51.11, 0.0, 0.0
    x_set_init, y_set_init = 3.93, -51.11
    v_init, t_init, A_init = 0, 0, 1
    x_ref_old, y_ref_old = 3.93, -51.11
    id, id2, L, lane_status, horizon, index, state, pre_state, phase = 23, 41, 2.995, 0, 6, 0, 0, 0, 0
    critical_dis, follow_dis, traj = 10, 10, 1
