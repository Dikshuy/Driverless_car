...
vinit, ainit, VMAX, num_foc, horizon, dyn_x, dyn_y, follow_dis, critical_dis, v_obs = 0, 0, 5.0, 10, 6, 103, 0, 10, 10, 0
...
def velocity(xv, yv, v_init, a_init, t_init, state):
    ...
    tim[0,0] = t_init
    a_long[0,0] = a_init
    # print(tim[0,0], tim[1,0],"print Kiya hai")
...
def Initialization2(vel, a_long, w, i, dist, itr2, vd_n, wd_n, ad_n, index, theta, theta4, itr4, ll):
    ...
    if(x[cc]< dyn_x and y[cc] == dyn_y):
        xf = [ float(x[cc]), float(dyn_x), float(x[c+1]) ]
        yf = [ float(y[cc]), float(dyn_y), float(y[c+1]) ]
        x_f = x[cc+1]
        y_f = y[cc+1]
    elif(x[cc]>dyn_x and y[cc] == dyn_y):
        ...
        xf = [ float(x[cc-1]), float(dyn_x), float(x[c+1]) ]
        yf = [ float(y[cc-1]), float(dyn_y), float(y[c+1]) ]
        x_f = x[cc+1]
        y_f = y[cc+1]
    elif(y[cc] != dyn_y):
