...
def velocity(xv, yv, v_init, a_init, t_init):
	global follow_dis, critical_dis, VMAX, v_obs
	state = 0
...
if __name__ == "__main__":
	global yaw, xact , yact, vact, wact
	yaw, xact, yact, vact, wact = 1.57488, -118.22, 3.46, 0.0, 0.0
	index = 0
	i = 0
	x_ref_old, y_ref_old = -118.22, 3.46
	...
	while(index < np.size(x)):
		xr, yr, vel, t, a_long, w= velocity(x[index:index+3], y[index:index+3], v_init, A_init, t_init)
		m_tang = ( y[index+1, 0] - y[index] )/ ( x[index + 1] - x[index] )

		if index == np.size(x) -6:		
			dist = math.sqrt((x[np.size(x)-1]-x[np.size(x)-6])**2 + (y[np.size(y)-1]-y[np.size(y)-6])**2)
		  	de_acc = (-speed_car.linear.x)/(2*abs(dist))
			speed_car.linear.x = speed_car.linear.x + 2 * de_acc * dist
