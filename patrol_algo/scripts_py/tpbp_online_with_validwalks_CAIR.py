...
def tpbp_walk(road_graph, algo_params, cur_node, priority_nodes, time_periods, assigned, folder_path, rob_id, num_robs):
	...
	#print 'Final Selected best walk is ' + str( best_walk), rob_id
...
class TPBP:
	...
	def team_update_CB(self, request):
		...
		if(len(self.robot_ids) < len(request.robot_id_list)):           #if a robot is added
			...
			if request.start_node_id in self.graph.node:
				self.robot_cur_walks[request.robot_id] = tpbp_walk(self.graph, self.algo_params, request.start_node_id, self.priority_nodes, self.time_periods, self.assigned, self.dest_folder, request.robot_id, len(self.robot_ids))
			else:
				for i in self.assigned:
					if self.assigned[i] == False:
						indx = i
				self.robot_cur_walks[request.robot_id] = tpbp_walk(self.graph, self.algo_params, self.priority_nodes[indx], self.priority_nodes, self.time_periods, self.assigned, self.dest_folder, request.robot_id, len(self.robot_ids))
	...
	def algorithm_setup(self):
		...
		for index, ith_robot in zip(range(len(self.robot_ids)), self.robot_ids):
			...
			self.robot_cur_walks[ith_robot] = tpbp_walk(self.graph, self.algo_params, self.priority_nodes[index], self.priority_nodes, self.time_periods, self.assigned, self.dest_folder, ith_robot, len(self.robot_ids))
			rospy.loginfo('PRINTING Dict')
			rospy.loginfo(self.robot_cur_walks)
		...
		for ith_robot in self.robot_ids:
			...
			try:
				...
				try:
					self.tpbp_to_pmm_proxy(self.task_list[ith_robot], ith_robot)
	...
	def task_done_CB(self, request):
		...
		if self.stamp <= request.stamp:# to ignore out-of-order messages from the simulator
			...
			'''
			#Log info for analysis
			print ("Start")
			print ('time-stamp', request.stamp)
			for node_name in self.graph.node:
				if node_name in self.priority_nodes:
					print 'priority_node', node_name, self.graph.node[node_name]['idleness']
				else:
					print 'non-priority', node_name, self.graph.node[node_name]['idleness']
			print ("End")
			'''
			...
			for i, itr_robot_id in zip(range(len(request.robot_id)),request.robot_id):
				...
				if len(self.robot_cur_walks[request.robot_id[i]]) == 0:
					...
					self.robot_cur_walks[itr_robot_id] = tpbp_walk(self.graph, self.algo_params, str(n), self.priority_nodes, self.time_periods, self.assigned, self.dest_folder, itr_robot_id, len(self.robot_ids))
					#rospy.loginfo('PRINTING Dict')
					#rospy.loginfo(self.robot_cur_walks)
