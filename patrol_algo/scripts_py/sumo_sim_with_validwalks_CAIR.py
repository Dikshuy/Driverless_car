...
import os
import glob
files = glob.glob('/home/dimple/sumo_ws/src/patrol_algo/heatmap_files/*')
for f in files:
    os.remove(f)
...
class Sumo_Sim:
    def __init__(self):
        ...
        self.heat_map_path = rospkg.RosPack().get_path('patrol_algo')+'/heatmap_files/'
        ...
        self.prev_last_edge = {}
        self.robots_edge_travel_list = {}

        for ith_robot in self.robot_ids:
            self.seq_of_routes[ith_robot] = []
            self.prev_last_edge[ith_robot]=''
            self.robots_edge_travel_list[ith_robot] = dict([(key, 0) for key in self.heat_map_key])  # Make a dictionary with keys equivalent to edge_ids
    ...
    def team_update_CB(self, request):
        ...
        if(len(self.robot_ids) < len(request.robot_id_list)):
            ...
            self.prev_last_edge[request.robot_id] = ''
            self.robots_edge_travel_list[request.robot_id] = dict([(key, 0) for key in self.heat_map_key])  # Make a dictionary with keys equivalent to edge_ids
    ...
    def simulation_startup(self, prior_nodes):
            ...
            for ith_robot in self.robot_ids:
                ...
                if ith_robot in vehicles:
                    ...
                    if last_edge in self.heat_map_key:
                        ...
                        if str(last_edge_update[ith_robot]) != str(self.prev_last_edge[str(ith_robot)]):
                            #print "Update....", self.robot_ids #start here
                            inc_count(self.robots_edge_travel_list[str(ith_robot)], str(self.prev_last_edge[str(ith_robot)]))
                            if self.prev_last_edge[str(ith_robot)] != '':
                                list_str_val = str(traci.simulation.getTime()) + ' '+ str(ith_robot) + ' '+str(self.prev_last_edge[str(ith_robot)])+'\n'
                            else:
                                list_str_val = str(traci.simulation.getTime()) + ' '+ str(ith_robot) + ' '+'None'+'\n'
                            robots_log.append(list_str_val)
                            #print ("here1", traci.simulation.getTime(), ith_robot, self.prev_last_edge[str(ith_robot)])
                    #else:
            ...
            if len(srvs.node_id) > 0:
                ...
                for vehicle_in_sumo in traci.vehicle.getIDList():
                    if vehicle_in_sumo not in self.robot_ids:
                        if vehicle_in_sumo == self.robot_remove_after_walk and self.rem_after_chk == True: #Remove after walk completition
                            ...
                            if traci.vehicle.isStopped(vehicle_in_sumo):
                                ...
                                del self.prev_last_edge[vehicle_in_sumo]
                                str_val = self.heat_map_path +"{}_edge_traversal.txt"
                                with open(str_val.format(vehicle_in_sumo), 'w') as f:
                                    f.write("%s\n" % self.robots_edge_travel_list[vehicle_in_sumo])
                                f.close()
                                del self.robots_edge_travel_list[vehicle_in_sumo]
                                self.num_vehicles = self.num_vehicles - 1
                        else:
                            print "In sumo:", vehicle_in_sumo, traci.vehicle.getRoadID(vehicle_in_sumo) #instant removal
                            traci.vehicle.remove(vehicle_in_sumo, 0x03)
                            del self.seq_of_routes[vehicle_in_sumo]
                            del self.updated[vehicle_in_sumo]
                            del self.last_nodes[vehicle_in_sumo]
        ...
        for vech in self.robot_ids:
            str_val = self.heat_map_path +"{}_edge_traversal.txt"
            with open(str_val.format(vech), 'w') as f:
                    f.write("%s\n" % self.robots_edge_travel_list[vech])
            f.close()

        path_log = self.heat_map_path+"log_stamp_edge.txt"
        with open(path_log, 'w') as f:
            for str_val_stamp in robots_log:
                f.write(str_val_stamp)
        f.close()
