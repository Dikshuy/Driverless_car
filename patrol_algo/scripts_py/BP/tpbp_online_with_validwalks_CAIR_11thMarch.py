...
class TPBP:
    ...
    def team_update_CB(self, request):
		...
		if self.robot_cur_walks[request.robot_id][-1] in self.priority_nodes:
			#print "here", request.robot_id, self.robot_cur_walks[request.robot_id][-1], '\n', self.robot_cur_walks[request.robot_id]
			#print "here1", self.graph.node[str(self.robot_cur_walks[request.robot_id][-1])]['idleness'], "check complete"
