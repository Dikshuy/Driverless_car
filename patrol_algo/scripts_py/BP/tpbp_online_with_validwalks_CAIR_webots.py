...
def tpbp_walk(road_graph, algo_params, cur_node, priority_nodes, time_periods, assigned, folder_path):
    ...
    if all([x == True for x in assigned]):
        idleness_value_pn = [road_graph.node[r]['idleness'] for r in priority_nodes]
        index_idleness = numpy.argmax(idleness_value_pn)
        valid_trails = '/valid_trails_{}_{}_{}.in'.format(cur_node, priority_nodes[index_idleness], str(int(time_periods[priority_nodes.index(cur_node)])))
        try:
            with open(folder_path + valid_trails, 'r') as f:
                count = 0
                for line in f:
...
class TPBP:
    ...
    def team_update_CB(self, request):
        ...
        if(len(self.robot_ids) < len(request.robot_id_list)):           #if a robot is added
            ...
            self.check_edges[request.robot_id] = tpbp_walk(self.graph, self.algo_params, self.priority_nodes[indx], self.priority_nodes, self.time_periods, self.assigned, self.dest_folder)
    ...
    def algorithm_setup(self):
        ...
        for index, ith_robot in zip(range(len(self.robot_ids)), self.robot_ids):
            ...
            self.check_edges[ith_robot] = tpbp_walk(self.graph, self.algo_params, self.priority_nodes[index], self.priority_nodes, self.time_periods, self.assigned, self.dest_folder)
        ...
        if self.stamp <= request.stamp:# to ignore out-of-order messages from the simulator
            ...
            for i, itr_robot_id in zip(range(len(request.robot_id)),request.robot_id):
                ...
                if str(n) == m:
                    ...
                    for node_values in self.check_edges['r0']:
                        check_edges_idleness.append(self.graph.node[node_values]['idleness'])
                        #print "here", request.robot_id[i], node_values, self.graph.node[node_values]['idleness']
                    print 'here', 'r0', check_edges_idleness, self.check_edges['r0']
                ...
                if len(self.robot_cur_walks[request.robot_id[i]]) == 0:
                    ...
                    self.check_edges[itr_robot_id] = tpbp_walk(self.graph, self.algo_params, str(n), self.priority_nodes, self.time_periods, self.assigned, self.dest_folder)
                    rospy.loginfo('PRINTING Dict')
                    rospy.loginfo(self.robot_cur_walks)
