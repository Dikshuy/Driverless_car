...
class TPBP:
    ...
    def timed_algo_setup_cb(self, timer):
        ...
        # self.algorithm_setup(self.dirname, self.folder, self.num_vehicles, self.priority_nodes, self.graph_name, self.algo, self.algo_params, self.time_periods)
    ...
    def team_update_CB(self, request):      
        ...
        if(len(self.robot_ids) < len(request.robot_id_list)):           #if a robot is added
            ...
            print("\nself.robot_cur_walks[request.robot_id][-1]:  " + str(self.robot_cur_walks[request.robot_id][-1]))
            print("\nself.priority_nodes:  " + str(self.priority_nodes))
            print("\nself.assigned:  " + str(self.assigned))
    ...
    def task_done_CB(self, request):
        ...
        #TODO robot have been removed before a task done message is received from simulator. (In actual, PMM will not send the task done message for a robot which has been removed.)
        ...
        if self.stamp <= request.stamp:# to ignore out-of-order messages from the simulator
            ...
            for i, itr_robot_id in zip(range(len(request.robot_id)),request.robot_id):
                ...
                if len(self.robot_cur_walks[request.robot_id[i]]) == 0: 
                    ...
                    #srvs = NextTaskSrv()
                    #pubs.task = map(int, self.robot_cur_walks[task_done_msg.robot_id[i]])
                    #srvs.task = map(str, self.robot_cur_walks[task_done_msg.robot_id[i]])
                    #srvs.robot_id = task_done_msg.robot_id[i]
                    print("SERVICE MESSAGE")
                    print(self.tpbp_to_pmm_proxy(map(str, self.robot_cur_walks[itr_robot_id]), itr_robot_id))
                    #self.list_robot_pub[task_done_msg.robot_id[i]].publish(pubs)
    ...
    # def remove_robot_id_from_team (self, robot_id):
    #     try:
    #         self.robot_ids.remove(robot_id)
    #         # TODO Remove the corresponding entry in all other arrays
    #     except ValueError:
    #         rospy.logerr(' robot id '+ robot_id + ' is not part of list of robot_ids for patrolling team')
    #         rospy.loginfo(' list of robot-ids is '+ str(self.robot_ids))
    
    # def add_robot_id_to_team (self, robot_id):
